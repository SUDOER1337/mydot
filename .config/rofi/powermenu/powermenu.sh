...
## Github : @adi1090x
## Edited by SUDOER1337 for Hyprland + Hyprlock support
#
## Rofi   : Power Menu
#
## Available Styles
...
run_cmd() {
    ...
    if command -v hyprctl &> /dev/null; then
        hyprctl dispatch exit || {
            notify-send "Rofi Powermenu" "Failed to exit Hyprland via hyprctl."
            exit 1
        }
    else
        notify-send "Rofi Powermenu" "hyprctl not found. Cannot logout Hyprland safely."
        exit 1
    fi
